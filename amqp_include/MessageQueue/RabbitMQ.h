...
public:
  ...
  /**
	...
	*  @deprecated 由于特殊的原因，这个函数可能会在将来的版本中取消。
	*/
   int32_t publish_ack(vector<CMessage> &message,string routkey, string &FailMessage, string &ErrorReturn=GetErr());

   int32_t publish_ack(CMessage &message,string routkey, string &FailMessage, string &ErrorReturn=GetErr());

   int32_t publish_ack(const string &message,string routkey, string &FailMessage, string &ErrorReturn=GetErr());
